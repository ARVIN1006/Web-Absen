...
<div class="admin-container">
    ...
    <div class="admin-header">
        <div>
            ...
            <p style="color: var(--text-muted); margin: 0.25rem 0 0;">Daftarkan anggota tim baru ke dalam sistem.</p>
        </div>
    </div>
        ...
        <div class="form-grid">
            ...
            <div class="form-section">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="name" class="form-input" value="{{ old('name') }}" required placeholder="Masukkan nama lengkap">
                    @error('name') <div class="error-text">{{ $message }}</div> @enderror
                </div>

                <div class="form-group">
                    <label class="form-label">Email Perusahaan</label>
                    <input type="email" name="email" class="form-input" value="{{ old('email') }}" required placeholder="email@perusahaan.com">
            ...
            <div class="form-section">
                    ...
                    <select name="work_shift_id" class="form-input">
                        ...
                        @foreach($workShifts as $shift)
                            <option value="{{ $shift->id }}" {{ old('work_shift_id') == $shift->id ? 'selected' : '' }}>
                                {{ $shift->name }} ({{ date('H:i', strtotime($shift->clock_in_time ?? $shift->start_time)) }} - {{ date('H:i', strtotime($shift->clock_out_time ?? $shift->end_time)) }})
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-top: 1.5rem;">
                    <div class="form-group">
                        ...
                        <input type="date" name="joined_at" class="form-input" value="{{ old('joined_at', date('Y-m-d')) }}">
                    </div>
                    <div class="form-group">
                        <label class="form-label">Akhir Kontrak</label>
                        <input type="date" name="contract_end_at" class="form-input" value="{{ old('contract_end_at') }}">
                    </div>
                </div>
            </div>
                ...
                <div class="form-grid" style="grid-template-columns: 1fr 2fr;">
                    <div>
                        <div class="form-group">
                            ...
                            <input type="text" name="phone_number" class="form-input" value="{{ old('phone_number') }}" placeholder="0812...">
                        </div>
                        <div class="form-group">
                            <label class="form-label">Nomor NPWP</label>
                            <input type="text" name="npwp" class="form-input" value="{{ old('npwp') }}" placeholder="00.000.000.0-000.000">
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Alamat Lengkap Sesuai Domisili</label>
                        <textarea name="address" class="form-input" rows="4" placeholder="Jl. Nama Jalan No. 123, Kelurahan, Kecamatan, Kota...">{{ old('address') }}</textarea>
                    </div>
                </div>
            </div>
        </div>
